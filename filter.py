        ...
        try:
            ...
            for customer in customers:
                if int(filter_string) == customer.get_id():
                    detected = True
                    lines = customer.get_phone_numbers()
            assert detected
            for call in data:
                if call.src_number in lines or \
                        call.dst_number in lines:
                    new_data.append(call)
        except ValueError:
            return data
        except TypeError:
            return data
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        try:
            ...
            assert filter_string[0] == "G" or filter_string[0] == "L"
            assert len(filter_string) != 0
            assert isinstance(filter_string, str)
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        try:
            assert isinstance(filter_string, str)
            assert len(filter_string) != 0
            assert len(filter_string.split(', ')) == 4
            ...
            assert map_max[0] >= lower_long >= map_min[0]
            assert map_max[1] >= lower_lat >= map_min[1]
            assert map_min[0] <= upper_long <= map_max[0]
            assert map_min[1] <= upper_lat <= map_max[1]
            for call in data:
                if (lower_long <= call.src_loc[0] <= upper_long and
                        lower_lat <= call.src_loc[1] <= upper_lat):
                    new_data.append(call)
                elif (lower_long <= call.dst_loc[0] <= upper_long and
