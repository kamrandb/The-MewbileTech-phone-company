...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    for event_data in log['events']:
        call_time = datetime.datetime.strptime(event_data["time"],
                                               "%Y-%m-%d %H:%M:%S")
        if call_time.month != billing_month or call_time.year != year:
        ...
        if event_data["type"] == "call":
            ...
            src_location = tuple(event_data["src_loc"])
            dst_location = tuple(event_data["dst_loc"])
            call = Call(src_number, dst_number, call_time, duration,
                        src_location, dst_location)
            caller_cust = find_customer_by_number(src_number, customer_list)
            called_cust = find_customer_by_number(dst_number, customer_list)
            if caller_cust is not None:
                caller_cust.make_call(call)
            if called_cust is not None:
                called_cust.receive_call(call)
